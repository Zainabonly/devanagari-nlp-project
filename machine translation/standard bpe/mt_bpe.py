...
BPE_TOKENIZER_PATH = "/DATA/rohit/NLP_2025/mt_bpe/bpe"   # folder with tokenizer.json
...
class Seq2SeqTransformer(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        out = self.transformer(
            ...
            # *** IMPORTANT FIX: prevent attending to encoder pads ***
...
def greedy_decode(src_tensor, max_len=80):
    ...
    model.eval()
    with torch.no_grad():
        src_pad_mask = (src_tensor == pad_id)  # (1, S)
        # encoder
        src_emb = model.pos_encoder(model.src_embedding(src_tensor) * math.sqrt(model.d_model))
        memory = model.transformer.encoder(src_emb, mask=None, src_key_padding_mask=src_pad_mask)
        # start with BOS
        ys = torch.tensor([[bos_id]], dtype=torch.long, device=src_tensor.device)
        for _ in range(max_len):
            dec_in = model.pos_encoder(model.tgt_embedding(ys) * math.sqrt(model.d_model))
            # causal mask for current length
            T = dec_in.size(1)
            tgt_mask = model.transformer.generate_square_subsequent_mask(T).to(dec_in.device)
            ...
            next_word = prob.argmax(dim=-1).unsqueeze(1)  # (1,1)
...
pretok_path = os.path.join(OUTPUT_DIR, "test_pretok_outputs_bpe.txt")
...
tok_path = os.path.join(OUTPUT_DIR, "test_tok_outputs_bpe.txt")
...
seq2seq_path = os.path.join(OUTPUT_DIR, "test_seq2seq_outputs_bpe.txt")

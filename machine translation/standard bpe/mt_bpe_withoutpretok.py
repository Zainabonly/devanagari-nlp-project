...
# If you prefer TXT split-with-random behavior, keep DATA_SRC/DATA_TGT and ignore CSV paths.
...
BPE_TOKENIZER_PATH = "/DATA/rohit/NLP_2025/mt_bpe/bpe"
...
# ========== TOKENIZER ==========
print("[INFO] Loading BPE tokenizer from:", BPE_TOKENIZER_PATH)
tokenizer = PreTrainedTokenizerFast.from_pretrained(BPE_TOKENIZER_PATH)
...
# ========== READ CSV SPLITS (headers: 'mix','english') ==========
def read_csv_pairs(path):
    ...
    with open(path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            s = row["mix"].strip() if row.get("mix") else ""
            t = row["english"].strip() if row.get("english") else ""
            if s and t:
                src.append(s)
                tgt.append(t)
    return src, tgt

print("[INFO] Reading CSV files for train/valid/test...")
...
train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate_fn) if len(train_ds) > 0 else []
valid_loader = DataLoader(valid_ds, batch_size=BATCH_SIZE, shuffle=False, collate_fn=collate_fn) if len(valid_ds) > 0 else []
...
class Seq2SeqTransformer(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        src_pad = (src == pad_id).to(DEVICE)
        tgt_pad = (tgt == pad_id).to(DEVICE)

        src_emb = self.pe(self.src_emb(src) * math.sqrt(self.d_model))
        tgt_emb = self.pe(self.tgt_emb(tgt) * math.sqrt(self.d_model))

        out = self.tf(src_emb, tgt_emb, tgt_mask=tgt_mask,
              src_key_padding_mask=src_pad,
              tgt_key_padding_mask=tgt_pad,
              memory_key_padding_mask=src_pad)   # <-- IMPORTANT FIX
        return self.fc(out)
...
total_steps = (len(train_loader) * EPOCHS) if (hasattr(train_loader, "__len__") and len(train_loader) > 0) else 1
# LinearLR requires start_factor, end_factor, total_iters style; use start_factor=1.0, end_factor=0.1
...
def train_epoch():
    ...
    for src_batch, tgt_batch in tqdm(train_loader, desc="Training"):
        ...
        optimizer.zero_grad()
        logits = model(src_batch, tgt_in)  # (B, T, V)
...
def compute_bleu(ds):
    ...
    for i in range(len(ds)):
        ...
        pred_tokens = [tokenizer.convert_ids_to_tokens(int(x)) for x in pred_ids
                       if int(x) not in (pad_id, bos_id, eos_id)]
        tgt_tokens = [tokenizer.convert_ids_to_tokens(int(x)) for x in tgt_ids
                      if int(x) not in (pad_id, bos_id, eos_id)]
        hyps.append(" ".join(pred_tokens).strip())
        refs.append([" ".join(tgt_tokens).strip()])
    bleu = sacrebleu.corpus_bleu(hyps, refs) if len(hyps) > 0 else type("X", (), {"score": 0.0})()
    score = bleu.score if hasattr(bleu, "score") else 0.0
...
for ep in range(1, EPOCHS + 1):
    ...
    # compute BLEU on validation (optional; can be slow)
...
orig_out_path = os.path.join(OUTPUT_DIR, "test_original_inputs_bpe.txt")
...
tok_out_path = os.path.join(OUTPUT_DIR, "test_tokenized_inputs_bpe.txt")
...
seq2seq_out_path = os.path.join(OUTPUT_DIR, "test_seq2seq_outputs_bpe.txt")
